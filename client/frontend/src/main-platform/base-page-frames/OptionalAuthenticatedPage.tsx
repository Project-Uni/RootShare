...
function OptionalAuthenticatedPage(props: Props) {
  ...
  const checkAuth = useCallback(async () => {
    if (Boolean(accessToken)) await checkProfilePictureExpired(dispatch);
    setLoading(false);
        ...
        {!loading && (
          <div className={styles.body} style={{ height: height }}>
            {width > showLeftEl.current &&
              (leftElement
                ? leftElement
                : Boolean(accessToken) && <MainNavigator />)}
            <div
              style={{
                flex: 1,
                overflow: 'scroll',
...
export default OptionalAuthenticatedPage;
