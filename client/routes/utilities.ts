...
module.exports = (app) => {
  ...
  // });
  app.get('/test/swaggerConvert', (req, res) => {
    const string = `const CommentSchema = new mongoose.Schema(
      {
        user: {required: true,},
    ...
    let list2 = [];
    let required = [];
    let schemaName = list[0].split(' ')[1].split('Schema')[0];
    schemaName = schemaName.charAt(0).toUpperCase() + schemaName.slice(1);
    for (let i = 0; i < list.length; i++) {
      const split = list[i].split(':');
      const item = split[0].trim();

      if (split.length > 1 && !['type', '{ timestamps'].includes(item))
        list2.push(item);

      if (list[i].includes('required')) required.push(item);
    }
    ...
    console.log(` *            description: Auto-generated ID`);
    ...
    if (string.includes('timestamps')) {
      console.log(` *          createdAt:`);
      console.log(` *            type: string`);
      console.log(` *            format: date-time`);
      console.log(` *            description: The date of the record creation.`);
      console.log(` *          updatedAt:`);
      console.log(` *            type: string`);
      console.log(` *            format: date-time`);
      console.log(` *            description: When the record was last updated`);
    }

    console.log(` *        example:`);
    console.log(` *          _id: 0123456789abcdef`);
    list2.forEach((element) => {
      console.log(` *          ${element}: `);
    });
